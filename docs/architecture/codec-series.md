...
- The shared fixture matrix (`fixtures/codec-series/matrix.json`, generated by
  `scripts/gen-series-fixtures.py`) runs through all three implementations:
  Rust (`crates/ndic-zarr/tests/series_matrix.rs`), Python
  (`bindings/python/nd-image-codecs/tests/test_codec_series_matrix.py`), and
