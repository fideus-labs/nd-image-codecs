...
- **py**: write the OME-Zarr lane's arrays with zarr-python
- address the CodeRabbit review on the 1.98 adoption PR
- **scripts**: stop --help truncating the re-measure script's header
...
- **codestream**: derive marker and packet offsets from their slices
- **htj2k**: make the DWT row splitter safe and deny unsafe_code

### 📚 Documentation

- **publishing**: narrow the pin claim, and check the scan by form
- correct four record claims the final verification pass could not back
- correct the publishing runbook where the packaging sweep contradicted it
- write the consumer-facing changelog entry for the 1.98 migration
- **bench**: record the baseline-refresh decision and its evidence
- **rust-198**: write the authoritative adoption summary
- correct the split_three cost to a 10-17 % gain
- **rust-198**: keep the SIMD lane and convert no further float sites
- **rust-198**: freeze the 1.98 benchmark and golden-vector baseline

### 📊 Benchmarks

- refresh the main baseline

### 🧪 Tests

- **ci**: check the commitizen scan by form rather than by count

### 📦 Build

- **ci**: pin every documented commitizen command to 4.17.0
- ship a README with every published crate
- raise the minimum supported Rust version from 1.91 to 1.98
